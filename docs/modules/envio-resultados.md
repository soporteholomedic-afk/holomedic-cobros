...
| `infrastructure/` | Adaptadores: SMTP, share UNC, SQL Server, CLI de PDFs, compresión de PDFs | `infrastructure/email/emailService.ts` (`EmailService` → `@/utils/sendEmail` con `purpose: 'consolidados'`), `infrastructure/files/UncFileRepository.ts` + `getFileRepository.ts` (singleton) + `patientPathResolver.ts`, `infrastructure/sqlserver/SqlServerEnvioHistoryRepository.ts` + `migrate.ts` + `getEnvioHistoryDb.ts` (pool `HOLOMEDIC`), `infrastructure/pdf/` (`PdfLibCompressorAdapter.ts` — compresión lossless fail-open, `PdfImageCompressorAdapter.ts` — perfil email: cirugía de imágenes DCT con sharp, downsample ÷2 a 150 DPI + JPEG q75, solo sobre la copia emaileada en RAM, `constants.ts` — kill switch `PDF_COMPRESSION_ENABLED`, perfil `PDF_COMPRESSION_PROFILE` y umbrales de elegibilidad), `infrastructure/informes/constants.ts` + `outputDirResolver.ts` + `parseManifest.ts` + `matchTransientAuthError.ts`, `infrastructure/mock/` (repos de desarrollo) |
...
| `PdfLibCompressorAdapter`, `PdfImageCompressorAdapter`, `PDF_COMPRESS_TIMEOUT_MS` | `infrastructure/pdf/` |
| `getPdfCompressionProfile` (perfil `lossless`\|`email`), `PDF_IMAGE_MIN_LONGEST_SIDE_PX` (1000), `PDF_IMAGE_MIN_DCT_STREAM_BYTES` (512 000), `PDF_IMAGE_RESIZE_DIVISOR` (2), `PDF_IMAGE_JPEG_QUALITY` (75) | `infrastructure/pdf/constants.ts` |
...
| Externa | `sharp` (npm, pnpm lockfile) | Perfil `email`: decode/resize/re-encode JPEG de imágenes DCT (prebuilds nativos libvips por plataforma — ver `serverExternalPackages` en `next.config.ts`) |
...
| `PDF_COMPRESSION_ENABLED` | Feature flag de compresión de PDFs en el envío (`'false'`/`'0'` la desactiva; ON por defecto; OFF = comportamiento legacy byte-idéntico, incluido el ordenamiento de tope legacy: 30 MB se verifican ANTES de cualquier compresión). **Precedencia absoluta**: si está OFF, ningún compresor se inyecta y el perfil es irrelevante | `infrastructure/pdf/constants.ts` |
| `PDF_COMPRESSION_PROFILE` | Selector del adaptador de compresión: `lossless` (DEFAULT, byte-safe) o `email` (downsample ÷2 a 150 DPI + JPEG q75 de imágenes DCT ≥1000 px y ≥500 KB en la copia emaileada — **REQUIERE aprobación clínica de legibilidad** para producción). Se lee por request (trim + lowercase; `' EMAIL '` ≡ `email`). Basura (`banana`) → `console.warn` + `lossless` (falla hacia fidelidad); unset/'' → `lossless` | `infrastructure/pdf/constants.ts` |
...
- `sharp` trae **prebuilds nativos por plataforma** (`@img/sharp-libvips-*` con `libvips-cpp.so`). Cualquier cambio de imagen Docker/standalone debe re-verificar que `require('sharp')` cargue dentro del contenedor (smoke `docker exec … node -e "require('sharp')"`) antes de habilitar el perfil `email` — el tracer de standalone no copia assets ELF que solo enlaza el binario nativo.
