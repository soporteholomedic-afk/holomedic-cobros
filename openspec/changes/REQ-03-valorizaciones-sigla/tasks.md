...
- [x] 1.1 `src/lib/db.ts` — RED first (`vi.stubEnv`: `sa` throws; fallback `DB_*`), then add a dedicated read-only pool getter (own env prefix falling back to `DB_*`, sa-rejection error class, test seam); widen `DbEnvPrefix` `[Q-R1|D1]` — **AMENDED (U5)**: the dedicated getter, sa guard and seam were REMOVED per requirement-author clarification (REQ-03 §3 governs AI-agent exploration, not the runtime pool); valoraciones now uses standard `getPool()` (`DB_*`) — see apply-progress U5
