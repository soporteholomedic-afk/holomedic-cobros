...
- [ ] 1.7 Rewrite `src/app/valoraciones/page.tsx` (`"use client"`); create `presentation/hooks/` (`useValoracionesFilters` useReducer, `useValoraciones`, `useLookup`) and `presentation/components/` (`FiltersPanel`, `ClienteAutocomplete`, `PacienteAutocomplete`, `EmpresaList` adapted from CompanyList, `EmpresaDetailModal`); periodo defaults today; consolidado disabled (slice 1); client→destinos gating, clearing resets both `[Q-R5|Q-R6]`
- [ ] 1.8 Delete `src/app/api/valoraciones/generate/**`, `src/utils/valoracionesCore.ts`, `src/utils/valoraciones.ts`, `src/components/CompanyList.tsx`, `src/components/CompanyDetailModal.tsx` + their tests (sole consumers removed by 1.7); RED: `/api/valoraciones/generate` → 404, no CSV code remains `[Q-R8]`
