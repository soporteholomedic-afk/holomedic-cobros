...
Feature-sliced `src/features/valoraciones/**` on the cobranza precedent: domain ports/entities, typed-bind SQL adapter over a new read-only SIGLA pool, cached-promise factory with test seams, user-safe routes, client page from hooks. PDF reuses `PdfPrinter`/`EdgePrinter` additively; Excel `xlsx`; email `sendEmail` + plantillas registry. 3 chained PR slices.

## Architecture Decisions

| # | Decision | Rejected | Rationale |
|---|---|---|---|
| D1 | `getSiglaReadOnlyPool()` in `src/lib/db.ts`: prefix `SIGLA_RO_*` falling back to `DB_*` (Holomedic-env pattern); DB `SIGLA_RO_NAME ?? DB_NAME ?? 'ICCGSA'`; lazy singleton; throws `SiglaRoSaError` pre-construction when user is `sa`; seam `__setSiglaRoPoolForTests` | reuse `getPool()` — `DB_USER` may be `sa` | OQ-1: read-only by construction; widens `DbEnvPrefix` |
...
## Data Flow
    ...
    FiltersPanel → GET /sigla → Repository → SP (RO pool) → rowToRepFacturacion (ISO)
...
| Target | Approach |
|---|---|
| Pool guard + Repository | `vi.stubEnv` (`sa` throws; fallback `DB_*`); fake rows (NULL `FecSTA`, `VImpMN/CodiEM` casing) → ISO entities |
...
No DB migration. Ops: `SIGLA_RO_*` env (or `DB_USER=explorar_datos`); verify EXPLORADOR_DATOS grants. Per-slice revert on `develop`.
