...
/**
 * GET /api/areas/musculoesqueletica/pacientes
 *
  ...
  try {
        ...
        WHERE O.IndReg = 1
          ...
          AND EXISTS (
            ...
            WHERE OS2.CodEmp = O.CodEmp
              ...
              AND E2.NomEsp = 'MUSCULO ESQUELETICA'
          )
        ORDER BY paciente
      `);
    ...
    if (rows.length > 0) {
      try {
        ...
        console.warn('areas/musculoesqueletica/pacientes: could not query evaluations from HOLOMEDIC');
      }
    }
    ...
    console.error('areas/musculoesqueletica/pacientes route error:', message);
    return NextResponse.json(
