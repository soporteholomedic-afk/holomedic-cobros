...
function usePacientes(codCli: number, fechaInicio: string, fechaFin: string) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/areas/medicina/pacientes?${params}`, { signal: controller.signal })
      .then(async (res) => {
        if (!res.ok) {
...
export default function MedicinaJjcPage() {
      ...
      <div className="flex items-center space-x-3 mb-2">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Pacientes del área de Medicina — Empresa JJC CONTRATISTAS GENERALES S.A.
          </p>
        </div>
      </div>
